...
# 1. Setup
old_setup = """# Set pin sebagai OUTPUT dan matikan awal-awal
...
# 2. Trigger
old_buzzer = """def trigger_buzzer(duration=0.5):
    try:
...
new_buzzer = """def trigger_buzzer(duration=0.5):
    try:
        print(f"[debug] Sounding buzzer for {duration}s")
        ...
        print(f"Buzzer Error: {e}")"""
c = c.replace(old_buzzer, new_buzzer)

# 3. Cleanup
old_cleanup = """    GPIO.output(BUZZER_PIN, GPIO.LOW)"""
